...
2. Run the CLI directly (no clone needed):
   ```sh
   uvx git+https://github.com/dunnkers/voice-memo-summarizer voice-memo-summarizer <file> [<file> ...]
   ```
3. If the user wants a custom prompt, pass `--prompt-file <path>` with a markdown file containing the prompt.
4. If the user wants to save the output to a file, redirect stdout: `uvx git+https://github.com/dunnkers/voice-memo-summarizer voice-memo-summarizer <file> > summary.md`.
